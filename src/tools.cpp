...
MatrixXd Tools::CalculateJacobian(const VectorXd& x_state) {
  ...
  if (fabs(px2py2) < 0.0001) {
    cout << "Tools::CalculateJacobian: Division by Zero detected!! Bailing out."
         << endl;
    return Hj;
  }
...
VectorXd Tools::CartesianToPolar(const VectorXd& cartesian) {
  ...
  // check division by (or close to) zero
  if (fabs(px2py2) < 0.0001) {
    cerr << "Tools::CartesianToPolar: Division by Zero detected!! Bailing out."
         << endl;
    exit(1);
  }
  ...
  if (cartesian.size() == 4)  // EKF
  {
    polar << sqrt_px2_py2, atan__py_div_px, (px * vx + py * vy) / sqrt_px2_py2;
  } else  // UKF
  {
...
void Tools::track_nis(Sensor_type sensor_type, const VectorXd& pred,
  ...
  switch (sensor_type) {
    case (RADAR): {
      ...
      double nis_radar = (nis_r_lower_than_95pcnt_ / radar_samp_count_)*100;
      cout << "Percentage of Radar NIS Samples below "<<nis_3d<<": "<< nis_radar << endl;
      break;
    }
    case (LASER): {
      ++laser_samp_count_;
      ...
      double nis_laser = (nis_l_lower_than_95pcnt_ / laser_samp_count_)*100;
      cout << "Percentage of Laser NIS Samples below "<<nis_2d<<": "<<nis_laser << endl;
      break;
    }
  }
}
